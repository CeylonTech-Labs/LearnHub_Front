...
export async function registerUser(input: {
  ...
  await sendEmail(
    ...
    `<p>Welcome to LearnHub. Verify your account using this code:</p><p><b>${token}</b></p>`
  );
...
export async function loginUser(email: string, password: string) {
  ...
  if (user.status === "SUSPENDED" || user.status === "REJECTED") throw new ApiError(403, "Account is not active");
...
export async function revokeRefreshToken(refreshToken?: string) {
  ...
  const match = found.find((token): token is (typeof tokens)[number] => token !== null);
  if (match) await prisma.refreshToken.update({ where: { id: match.id }, data: { revokedAt: new Date() } });
}
